...
fn plot(y: &[f32], color: Color) {
    ...
    for k in 1..y.len() {
        ...
        draw_line(prev_x, prev_y, px, py, 2., color);
        prev_x = px;
        prev_y = py;
    }
}
...
async fn main() {
    ...
    let x: Vec<Complex> = (0..samples).map(
        |x| Complex::new(
            ...
            f32::sin(2. * PI * (x as f32 / samples as f32 * time) * 3.)
        )
    ).collect();
    ...
    let peaks: Vec<usize> = fft::util::peaks(yout.as_slice());
    for peak in peaks {
        println!("Peak at index {}, {} Hz", peak, fft::util::k_to_hz(peak, samples, time));
    }
